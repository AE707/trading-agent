...
export default function Dashboard() {
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900 text-gray-100">
      ...
      <div className={`fixed md:relative z-50 h-screen bg-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-0 md:w-0'}`}>
        <aside className="hidden md:flex flex-col gap-3 p-4">
          <nav className="space-y-2">
            {['Overview', 'Metrics', 'Predictions', 'Trades', 'Analytics', 'Settings'].map((item) => (
              <div key={item} className="px-4 py-2 rounded hover:bg-slate-700 cursor-pointer transition-colors">
                {item}
              </div>
            ))}
          </nav>
      ...
      <main className="flex-1 overflow-auto bg-gradient-to-b from-slate-900 via-slate-950 to-slate-900">
        ...
        <header className="sticky top-16 z-30 bg-slate-800/95 backdrop-blur border-b border-slate-700 p-4">
          <div className="flex items-center justify-between gap-2">
            <button onClick={() => setSidebarOpen(!sidebarOpen)} className="p-2 hover:bg-slate-700 rounded transition-colors flex-shrink-0">
              ☰
            </button>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6 space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-3">
            ...
            <MetricsCard title="Time" value={`${metrics.avgPredictionTime}ms`} change="-8.3%" icon="⏱️" />
          </div>
            ...
            <div className="bg-slate-800 rounded-lg p-4 sm:p-6 border border-slate-700 shadow-lg hover:shadow-xl transition-shadow">
              ...
              <ResponsiveContainer width="100%" height={250}>
                <PieChart data={featureData} margin={{ top: 5, right: 20, bottom: 5, left: -20 }}>
                  <Pie data={featureData} cx="50%" cy="50%" outerRadius={60} fill="#888848" dataKey="value">
                ...
                {phaseData.map((phase, idx) => (
                  <div key={idx} className="flex items-center gap-2 text-xs sm:text-sm">
                    <span className="flex-1 bg-slate-700 h-2 rounded-full overflow-hidden min-w-0">
                      <div className="bg-gradient-to-r from-blue-500 to-cyan-500 h-full rounded-full overflow-hidden min-w-0" style={{ width: `${phase.accuracy}%` }} />
                    </span>
          ...
          <div className="flex flex-col sm:flex-row gap-4 mb-6">
            <button onClick={() => exportToCSV(filteredPipeline, 'ml-pipeline.csv')} className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
              📊 Export CSV
            </button>
            <button onClick={() => exportToPDF(filteredPipeline, 'ML Pipeline Results')} className="flex-1 px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-colors">
              📄 Export PDF
            </button>
            <ColumnManager columns={[{ id: 'phase', label: 'Phase', visible: true }, { id: 'accuracy', label: 'Accuracy', visible: true }, { id: 'tests', label: 'Total Tests', visible: true }, { id: 'success', label: 'Success Rate', visible: true }]} onColumnChange={() => {}} />
          </div>
          ...
          {/* Advanced Analytics */}
          <div className="bg-slate-800 dark:bg-slate-800 rounded-lg shadow-md p-6 mt-6 border border-slate-700">
